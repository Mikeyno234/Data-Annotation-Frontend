...
export const toolsByModality: Record<ModalityType, ToolOption[]> = {
  IMAGE: [
    ...
    },
    {
      ...
      defaultXml: (labels) => `<View><Choices name="choice" toName="image">${labels.map((l) => `<Choice value="${l.name}" background="${l.color}"/>`).join('')}</Choices><Image name="image" value="$image_url"/></View>`,
  ...
  TEXT: [
    ...
    },
    {
      ...
      defaultXml: (labels) => `<View><Choices name="sentiment" toName="text">${labels.map((l) => `<Choice value="${l.name}" background="${l.color}"/>`).join('')}</Choices><Text name="text" value="$text"/></View>`,
  ...
  VIDEO: [
    ...
    },
    {
      ...
      defaultXml: (labels) => `<View><Choices name="action" toName="video">${labels.map((l) => `<Choice value="${l.name}" background="${l.color}"/>`).join('')}</Choices><Video name="video" value="$video_url"/></View>`,
